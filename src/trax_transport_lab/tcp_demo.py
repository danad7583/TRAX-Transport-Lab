...
def _server(
    ...
    try:
        ...
        init = decode_message(recv_frame(conn, MAX_PACKET_LEN))
        ...
        send_frame(conn, encode_message(init_ack))
        log.add("TRAX_INIT_ACK sent")

        commit = decode_message(recv_frame(conn, MAX_PACKET_LEN))
        ...
        req = decode_message(recv_frame(conn, MAX_PACKET_LEN))
        ...
        send_frame(conn, encode_message(req_ack))
        log.add("TRAX_REQ_ACK sent")

        junk = decode_message(recv_frame(conn, MAX_PACKET_LEN))
        ...
        send_frame(conn, encode_message(res_ack))
...
def run_tcp_demo(adverse_case: str | None = None, adapter: TraxAdapter | None = None) -> TcpDemoResult:
    ...
    server_thread = threading.Thread(
        target=_server, args=(listener, adapter, dag, log, server_keys), daemon=True
    )
    server_thread.start()

    error: str | None = None
    try:
        _client(host, port, adapter, log, adverse_case, server_keys.public_key)
    except ProtocolError as exc:
        log.reject(exc.message_type, exc.reason)
        error = exc.reason
    except (MessageError, FramingError, OSError, KeyError, ValueError) as exc:
...
def _client(
    ...
    with socket.create_connection((host, port), timeout=SOCKET_TIMEOUT_SECONDS) as sock:
        ...
        if adverse_case == "malformed_init":
            ...
            log.add("TRAX_INIT sent")
            return
        send_frame(sock, encode_message(init))
        log.add("TRAX_INIT sent")
        ...
        if adverse_case == "req_before_commit":
            ...
            send_frame(sock, encode_message(bad_req))
            log.add("TRAX_REQ sent")
            return

        init_ack = decode_message(recv_frame(sock, MAX_PACKET_LEN))
        ...
        send_frame(sock, encode_message(commit))
        ...
        if adverse_case == "payload_before_ack":
            ...
            send_frame(sock, encode_message(junk))
        ...
        send_frame(sock, encode_message(req))
        log.add("TRAX_REQ sent")
        if adverse_case == "wrong_message_order":
            return
        ...
        req_ack = decode_message(recv_frame(sock, MAX_PACKET_LEN))
        req_ack_payload = security_payload(
            "TRAX_REQ_ACK",
            {
                "cycle_index": 0,
        ...
        res_ack = decode_message(recv_frame(sock, MAX_PACKET_LEN))
